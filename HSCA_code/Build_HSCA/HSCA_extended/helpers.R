...
remove_LQ_cells <- function(seurat_obj) {
  has_lvl4 <- "inherited_celltype_lvl_4_extended" %in% colnames(seurat_obj[[]])
  has_lvl5 <- "inherited_celltype_lvl_5_extended" %in% colnames(seurat_obj[[]])

  total_cells <- ncol(seurat_obj)
  keep <- rep(TRUE, total_cells)

  if (has_lvl4) {
    keep <- keep & !grepl("LQ", seurat_obj$inherited_celltype_lvl_4_extended)
  }
  if (has_lvl5) {
    keep <- keep & !grepl("LQ", seurat_obj$inherited_celltype_lvl_5_extended)
  }
...
clean_lvl5_numeric <- function(object) {
  if ("inherited_celltype_lvl_5_extended" %in% colnames(object@meta.data)) {
    object$inherited_celltype_lvl_5_extended[
      grepl("^\\d+$", object$inherited_celltype_lvl_5_extended)
    ] <- NA
  }
  return(object)
}
...
inherit_lvl4_to_lvl5 <- function(seurat_obj) {
  if (!"inherited_celltype_lvl_5_extended" %in% colnames(seurat_obj@meta.data)) {
    seurat_obj$inherited_celltype_lvl_5_extended <- NA
  }

  # Only for cells where inherited_celltype_lvl_5 is still empty
  empty_lvl5 <- is.na(seurat_obj$inherited_celltype_lvl_5_extended) | seurat_obj$inherited_celltype_lvl_5_extended == ""

  lvl4 <- seurat_obj$inherited_celltype_lvl_4_extended[empty_lvl5]
  ...
  seurat_obj$inherited_celltype_lvl_5_extended[empty_lvl5] <- new_lvl5
